...
annopath = os.path.join(args.voc_root, 'VOC2021', 'Annotations', '%s.xml')
imgpath = os.path.join(args.voc_root, 'VOC2021', 'JPEGImages/detect', '%s.jpg')
imgsetpath = os.path.join(args.voc_root, 'VOC2021', 'ImageSets',
                          'Main', '{:s}.txt')
YEAR = '2021'
...
if __name__ == '__main__':
    ...
    weight = torch.load('./weights/VOC.pth')
    ...
    net = build_ssd('test', 300, 64)
    net.load_state_dict(new_weights)
    torch.save(net.state_dict(), './weights/VOC.pth')
    # load net
    ...
    # load data
    dataset = VOCDetection(args.voc_root, [('2021', set_type)],
                           BaseTransform(args.input, dataset_mean),
                           VOCAnnotationTransform(),train=False)
    if args.cuda:
        net = net.cuda()
