...
int main()
{
    ...
    for (i = 0; i < ny; i++)
    {
        for (j = 0; j < nx; j++)
        {
            ...
            {
                outimageR[i][j] = inputimageR[i - 1][j - 1] + inputimageR[i - 1][j] + inputimageR[i - 1][j + 1] + inputimageR[i][j - 1] + inputimageR[i][j] + inputimageR[i][j + 1] + inputimageR[i + 1][j - 1] + inputimageR[i + 1][j] + inputimageR[i + 1][j + 1];
                outimageG[i][j] = inputimageG[i - 1][j - 1] + inputimageG[i - 1][j] + inputimageG[i - 1][j + 1] + inputimageG[i][j - 1] + inputimageG[i][j] + inputimageG[i][j + 1] + inputimageG[i + 1][j - 1] + inputimageG[i + 1][j] + inputimageG[i + 1][j + 1];
                outimageB[i][j] = inputimageB[i - 1][j - 1] + inputimageB[i - 1][j] + inputimageB[i - 1][j + 1] + inputimageB[i][j - 1] + inputimageB[i][j] + inputimageB[i][j + 1] + inputimageB[i + 1][j - 1] + inputimageB[i + 1][j] + inputimageB[i + 1][j + 1];

                outimageR[i][j] = outimageR[i][j] / 9.0;
                outimageG[i][j] = outimageG[i][j] / 9.0;
                outimageB[i][j] = outimageB[i][j] / 9.0;
            }
