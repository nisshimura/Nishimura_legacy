...
int main()
{
    ...
    printf("Input fmin : ");
    scanf("%f", &fmin);
    printf("Your input fmax:%d,fmin:%d\n", fmax, fmin);

    for (i = 0; i < ny; i++)
    {
        for (j = 0; j < nx; j++)
        {
            outimageR[i][j] = (inputimageR[i][j]-fmin)*255/(fmax-fmin); /// inputimageの値をそのままoutimageへ格納
            outimageG[i][j] = (inputimageG[i][j] - fmin) * 255 / (fmax - fmin);
